export const MSG_ERR = {
  MSG_ERR_01:
    "Les deux mots de passe ne sont pas identiques. Veuillez les ressaisir.",
  MSG_ERR_02:
    "Le service en test « FranceTransfert » est réservé aux échanges de fichiers/dossiers volumineux avec l’administration d’état (à destination d’adresses de courriels se terminant par @xxx.gouv.fr) @xxx.gouv@xxx.gouv.",
  MSG_ERR_03:
    "Il est nécessaire de télécharger au moins un fichier ou un dossier pour démarrer le transfert.",
  MSG_ERR_04:
    "Il semble qu’au moins une des adresses de courriels soit erronée. Merci de vérifier.",
  MSG_ERR_06:
    "Merci de renseigner au moins un destinataire pour votre transfert.",
  MSG_ERR_07: "Merci de renseigner votre adresse de courriel."
};
